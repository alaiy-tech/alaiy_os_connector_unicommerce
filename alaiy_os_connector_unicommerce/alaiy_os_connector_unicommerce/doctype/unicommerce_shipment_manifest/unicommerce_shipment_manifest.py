...
from alaiy_os_connector_unicommerce.unicommerce.client.manifest import create_and_close_shipping_manifest
from alaiy_os_connector_unicommerce.unicommerce.constants import (
    CHANNEL_ID_FIELD, FACILITY_CODE_FIELD, INVOICE_CODE_FIELD, MANIFEST_GENERATED_CHECK, ORDER_CODE_FIELD,
    SHIPPING_PACKAGE_CODE_FIELD, SHIPPING_PROVIDER_CODE, TRACKING_CODE_FIELD,
)
from alaiy_os_connector_unicommerce.unicommerce.fulfillment.invoice import fetch_pdf_as_base64
from alaiy_os_connector_unicommerce.unicommerce.utils import remove_non_alphanumeric_chars

# invoice field -> manifest package field
FIELD_MAPPING = {
...
class UnicommerceShipmentManifest(Document):
    ...
    def set_unicommerce_details(self):
        """Populate the packages table from each invoice's details and validate them."""
        for package in self.manifest_items:
            ...
            if self.channel_id != package_info.get(CHANNEL_ID_FIELD):
                frappe.throw(
                    _("Row #{0}: Only {1} channel packages can be added in this manifest").format(
                        package.idx, self.channel_id
                    )
                )

            if cint(package_info.get(MANIFEST_GENERATED_CHECK)):
                frappe.throw(_("Row #{0}: Manifest is already generated, please remove package.").format(package.idx))
    ...
    def get_facility_code(self) -> str:
        ...
        if len(facility_codes) != 1:
            frappe.throw(_("Shipping manifest should only have one facility code, found: {0}").format(",".join(facility_codes)))
    ...
    def create_and_close_manifest_on_unicommerce(self):
        ...
        response = create_and_close_shipping_manifest(
            client,
        ...
        self.unicommerce_manifest_id = status.get("id")
...
def get_sales_invoice_details(sales_invoice):
    si_data = frappe.db.get_value(
        ...
        [
            "shipping_address", CHANNEL_ID_FIELD, FACILITY_CODE_FIELD, ORDER_CODE_FIELD,
            SHIPPING_PACKAGE_CODE_FIELD, SHIPPING_PROVIDER_CODE, TRACKING_CODE_FIELD, MANIFEST_GENERATED_CHECK,
        ],
        as_dict=True,
    )

    items = frappe.db.get_values("Sales Invoice Item", {"parent": sales_invoice}, "item_name", as_dict=True)
    si_data["item_list"] = ",".join({item.item_name for item in items})
    return si_data
